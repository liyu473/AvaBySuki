...
public class ThemeConfigService : IThemeConfigService
{
    ...
    public async Task<ThemeSettings?> LoadSettingsAsync()
    {
        try
        {
            ...
            {
                _logger.LogWarn("主题配置文件不存在，返回默认设置");
                return null;
            }
            ...
            _logger.LogInfo("成功加载主题设置");
            return settings;
        }
        catch (Exception ex)
        {
            _logger.LogErr(ex, "加载主题设置失败");
            return null;
        }
    }
    ...
    public async Task SaveSettingsAsync(ThemeSettings settings)
    {
        try
        {
            ...
            _logger.LogInfo($"成功保存主题设置到: {_settingsFilePath}");
        }
        catch (Exception ex)
        {
            _logger.LogErr(ex, "保存主题设置失败");
        }
    }
}