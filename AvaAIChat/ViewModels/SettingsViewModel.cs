...
public partial class SettingsViewModel : ViewModelBase
{
    ...
    private void ToggleTheme()
    {
        ...
        _logger.LogInfo($"主题已切换到: {CurrentThemeName}");
    ...
    private void ApplyCustomColor()
    {
        ...
        _logger.LogInfo($"应用自定义颜色: {CustomColor}");
    }
    ...
    private void ShowAddCustomColorDialog()
    {
        ...
        try
        {
            ...
            _logger.LogInfo($"初始化颜色选择器，颜色: {DialogCustomColor}");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "初始化颜色选择器失败");
        }

        // 监听颜色变化
        colorPicker.PropertyChanged += (sender, e) =>
        {
            if (e.Property == ColorPicker.StandardColorPicker.ColorProperty &&
                e.NewValue is ColorPicker.Models.NotifyableColor notifyableColor)
            {
                try
                {
                    ...
                    _logger.LogInfo($"颜色已更新: {newColor}");
                }
                catch (Exception ex)
                {
        ...
        _dialogManager.CreateDialog()
            ...
            .WithActionButton("应用", _ =>
            {
                try
                {
                    if (colorPicker.Color is ColorPicker.Models.NotifyableColor finalColor)
                    {
                        DialogCustomColor = Color.FromArgb(255,
                            (byte)finalColor.RGB_R,
                            (byte)finalColor.RGB_G,
                            (byte)finalColor.RGB_B);
                        _logger.LogInfo($"从颜色选择器读取颜色: {DialogCustomColor}");
                    }
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "读取颜色选择器颜色失败");
                }

                _logger.LogInfo($"点击应用按钮，应用颜色: {DialogCustomColor}");
    ...
    private void AddCustomColorToTheme()
    {
        ...
        if (existingInTheme != null)
        {
            ...
            _logger.LogInfo($"已存在主题，直接切换: {existingInTheme.DisplayName}");
            _ = SaveSettingsAsync();
            return;
        }
        ...
        _logger.LogInfo($"添加并应用自定义颜色: {DialogCustomColor}");
        _ = SaveSettingsAsync();
    }
    ...
    private void DeleteColorTheme(SukiColorTheme? colorTheme)
    {
        ...
        {
            _logger.LogInfo($"无法删除系统自带主题: {colorTheme.DisplayName}");
            return;
        }
        ...
        _logger.LogInfo($"已删除自定义颜色主题: {colorTheme.DisplayName}");
    ...
    partial void OnSelectedColorThemeChanged(SukiColorTheme? value)
    {
        if (value != null)
        {
            ...
            _logger.LogInfo($"主题颜色已切换到: {value.DisplayName}");
